...
param ghz;
param power_phys := energy_per_wire * ghz * wires_per_mc;
param power_ctrl;
...
var max_wire_mem;
s.t. def_max_wire_mem: max_wire_mem == sqrt((sum {i in Components} component_counts[i] * component_areas[i]) / 6) * 6 * package_layer / link_pitch;
...
s.t. wire_constraint: max_wire_mem >= component_counts['mc'] * wires_per_mc;
